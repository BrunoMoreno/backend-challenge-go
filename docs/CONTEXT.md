...
- Fase: **M6 completo** — worker de referências (`internal/infra/referenceworker`) com claim `FOR UPDATE SKIP LOCKED`, resolução tardia transacional (lock da linha = lease), retry com backoff exponencial (`internal/platform/backoff`, compartilhado com a outbox), TTL/limite de tentativas → `REFERENCE_NOT_FOUND`, varredor de `PENDING` órfãos com referência, dois workers disputando resolvem uma vez (M6.4), métrica `reference_resolutions_total`. Ao longo do caminho: retry de deadlock no `Process` (corrida M3.6) e `FindPendingDue`/interface `storage` ampliadas. Próximo marco: **M9 — Multi-instância e falhas** (M4.7 fica como pendência).
- Última tarefa concluída: 6.4 (`TEST` referência tardia, reinício no meio, expiração)
- Próxima tarefa: 9.1 (`ADD` build tag `faultinject` e pontos de crash)
- Bloqueios: —
