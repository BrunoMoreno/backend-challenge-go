...
- Fase: **M9 concluído** — harness multi-instância com 3 processos independentes do binário (`test/e2e`, build tag `faultinject`): disputa 100.00 × 2×80.00 entre instâncias → 1 `PROCESSED` + 1 `INSUFFICIENT_FUNDS` + saldo 20.00 + 1 débito (`TestDisputeAcrossInstances`); 50 envios idênticos em paralelo → 1 débito (`TestFiftyDuplicatesAcrossInstances`); crash do consumidor pós-commit pré-delete com reentrega sem duplicação (`TestCrashConsumerAfterCommit`); conferência final saldo armazenado = Σ do ledger + reconciliação consistente (`TestFinalConferencia`). Suíte **determinística e verde com `-race`** (4 execuções consecutivas `-count=1`). Junto aos bugs reais que a suíte caçou e fixou: 500 no `GET /wallets/{id}/ledger` (scan de named type em `pgx`, parâmetro SQL não usado `$2` e sentinela de cursor `TIMESTAMPTZ 'epoch'` ≠ zero do Go em `internal/infra/postgres/ledger.go`).
